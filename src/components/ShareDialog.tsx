...
export default function ShareDialog({ content, onClose }: ShareDialogProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const sharePassword = !isPublic && password ? password : null;
  ...
  const handleCopy = async () => {
    try {
      const textToCopy = sharePassword 
        ? `${shareUrl}\n密码: ${sharePassword}` 
        : shareUrl;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
          ...
          {shareUrl && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={handleCopy}
                  className="px-4 py-2 text-sm bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded whitespace-nowrap"
                >
                  {copied ? t('copied') : t('copyLink')}
                </button>
              </div>
              {sharePassword && (
                <div className="px-3 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded text-sm text-amber-700 dark:text-amber-300">
                  {t('passwordInfo')}: <strong>{sharePassword}</strong>
                </div>
              )}
            </div>
          )}
